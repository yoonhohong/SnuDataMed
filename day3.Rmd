...
오늘은 데이터 정제(cleaning)와 가공에 대해 학습합니다. 본격적인 데이터 분석에 앞선 전처리 과정입니다(preprocessing). 다음으로 데이터를 시각화하는 방법에 대해 알아봅니다.     
...
데이터 가공이란 원시 데이터를 분석하거나 시각화하기에 앞서 소프트웨어가 처리할 수 있는 적절한 형식으로 전환하는 과정을 말합니다(the process of transforming and mapping data from one "raw" data form into another format with the intent of making it more appropriate and valuable for a variety of downstream purposes such as analytics)
...
gapminder 데이터셋에는 국가별로 년도별 life expectancy, GDP per capita, population 등의 데이터가 포함되어 있습니다.       
...
데이터 작업을 위해 외부 데이터를 불러들이는 방법에 대해서 알아봅니다.  
...
write.table(demograph, file = "demograph_roundage.txt", row.names = F, quote = F)
```
